...
export default function TerminalView() {
  ...
  useEffect(() => {
    if (terminalVisible && terminals.length === 0 && projectPath !== null) {
      ...
      <div className="terminal-body">
        {panelTab === 'terminal' && (
          <div className={`terminal-panes${splitPair ? ' split' : ''}`}>
            ...
            terminals.map(term => {
              const isVisible = splitPair ? splitPair.includes(term.id) : term.id === activeTerminalId
              return (
                <TerminalPane
                  key={term.id}
                  id={term.id}
                  active={panelTab === 'terminal' && isVisible}
                />
              )
            })
          )}
