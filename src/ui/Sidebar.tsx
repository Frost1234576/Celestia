...
function TreeNode({ node, depth, activePath, onFileClick, onContextMenu, onRename, onDelete, onNewFile, onNewFolder }: {
  ...
  const handleClick = async () => {
    if (renaming) return
    if (isExpandable) {
      if (!expanded && (isVirtual || node.isArchive) && !children?.length) {
        setLoading(true)
        try {
  ...
  return (
    <div className="tree-node">
      ...
      >
        <span className={`tree-arrow${(isExpandable && ((children !== undefined && children?.length > 0) || node.isArchive || isVirtual)) ? (expanded ? ' open' : loading ? ' loading' : '') : ' hidden'}`}>
          {loading ? '◌' : '▶'}
        </span>
        <FileIcon
          name={node.name}
        ...
        {(!isVirtual || node.isArchive) && hovered && (
          <span className="tree-actions" onClick={e => e.stopPropagation()}>
            {isExpandable && !isVirtual &&(
              <>
                <span
...
export default function Sidebar() {
  ...
  return (
    <div className="sidebar">
      ...
      {contextMenu && buildContextItems(contextMenu.node).length > 0  &&(
        <ContextMenu
          x={contextMenu.x}
          y={contextMenu.y}
