...
import { registerKotlinLanguage } from './kotlinLanguage'
...
function lintModel(monacoApi: typeof monaco, model: monaco.editor.ITextModel): void {
  ...
  if (lang === STELLA_LANGUAGE_ID) {
    ...
    // Kotlin diagnostics handled by LSP - skip local linting
    monacoApi.editor.setModelMarkers(model, 'kotlin-linter', [])
  } else {
...
export function setupLinting(monacoApi: typeof monaco): void {
  ...
  registerKotlinLanguage(monacoApi)
